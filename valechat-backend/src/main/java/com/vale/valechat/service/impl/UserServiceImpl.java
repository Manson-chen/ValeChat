...
//        channelList.retainAll(userChannelList);
    ...
    @Override
    public ChatListVO getUnreadChatList(UserChatListRequest userChatListRequest) {
        ...
        for(UserVO userVO : chatListVO.getUserList()){
            ...
            if(message != null) {
                ...
                Date userLastReadTime = redisService.getLastReadTime("user" + userId, "u" + userVO.getId());
        ...
        for(Channel channel: chatListVO.getChannelList()){
            ...
            if(message != null) {
                Date chatLastMessageTime = message.getCreateTime();
                Date userLastReadTime = redisService.getLastReadTime("user" + userId, "c" + channel.getId());
                if ((userLastReadTime == null) || userLastReadTime.compareTo(chatLastMessageTime) < 0) {
        ...
        for(OrgChannelVo orgChannelVo: chatListVO.getOrgChannelList()){
            ...
            if(message != null) {
                Date chatLastMessageTime = message.getCreateTime();
                Date userLastReadTime = redisService.getLastReadTime("user" + userId, "c" + orgChannelVo.getChannelId());
                if ((userLastReadTime == null) || userLastReadTime.compareTo(chatLastMessageTime) < 0) {
